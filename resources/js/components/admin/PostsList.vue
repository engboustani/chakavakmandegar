                ...
                <datatable :columns="columns" :data="getData" :filter-by="filter">
                    <template slot-scope="{ row }">
                        <tr>
                            <td>{{ row.id }}</td>
                            <td>{{ row.title }}</td>
                            <td><el-button type="success" icon="el-icon-check" v-if="row.visiable" circle></el-button><el-button type="info" icon="el-icon-close" v-if="!row.visiable" circle></el-button></td>
                            <td>{{ farsiDatetime(row) }}</td>
                            <td>{{ farsiUpdate(row) }}</td>
                            <td>
...
export default {
    data: function () { return {
        columns: [
            ...
            {label: 'آخرین آپدیت', representedAs: this.farsiUpdate, align: 'right'},
    ...
    methods: {
        ...
        farsiUpdate: function(row) {
            return this.constructor(moment(row.updated_at).locale('fa').format('YYYY/M/D H:m'));
        },
        ...
        deletee(row) {
            this.$confirm('آیا مطمئن هستید که میخواهید ایونت را حذف کنید؟?', 'اخطار', {
                    ...
                    axios({url: '/api/post/delete', data: {id: row.id}, method: 'DELETE' })
                        .then(resp => {
                            this.$message({
                                type: 'success',
